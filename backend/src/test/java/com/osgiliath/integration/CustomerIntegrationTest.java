...
/** Integration tests for Customer API endpoints Tests full CRUD operations with real database */
...
class CustomerIntegrationTest extends BaseIntegrationTest {
    ...
    void shouldFailToCreateCustomerWithDuplicateEmail() throws Exception {
        ...
        Customer existingCustomer =
                testDataBuilder.customer().email("duplicate@example.com").buildAndSave();

        CreateCustomerCommand command =
                new CreateCustomerCommand("Another Customer", "duplicate@example.com", null, null);
    ...
    void shouldFailToCreateCustomerWithInvalidEmail() throws Exception {
        // Given
        CreateCustomerCommand command =
                new CreateCustomerCommand("John Doe", "not-an-email", null, null);
    ...
    void shouldFailToCreateCustomerWithEmptyName() throws Exception {
        // Given
        CreateCustomerCommand command =
                new CreateCustomerCommand("", "john@example.com", null, null);
    ...
    void shouldReturn404WhenCustomerNotFound() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/customers/" + nonExistentId)).andExpect(status().isNotFound());
    }
    ...
    void shouldReturn404WhenDeletingNonExistentCustomer() throws Exception {
        ...
        // When & Then
        mockMvc.perform(delete("/api/customers/" + nonExistentId)).andExpect(status().isNotFound());
    }
