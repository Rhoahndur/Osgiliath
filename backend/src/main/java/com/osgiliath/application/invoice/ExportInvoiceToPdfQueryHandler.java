...
/** Handler for ExportInvoiceToPdfQuery Generates a professional PDF document for an invoice */
@Service
...
public class ExportInvoiceToPdfQueryHandler {
    ...
    private void addHeader(Document document) {
        ...
        Paragraph companyName =
                new Paragraph("Osgiliath ERP System")
                        .setFontSize(14)
                        .setTextAlignment(TextAlignment.CENTER)
                        .setMarginBottom(20);
    ...
    private void addLineItemsTable(Document document, Invoice invoice) {
        ...
        for (LineItem item : invoice.getLineItems()) {
            ...
            table.addCell(
                    createCell(
                            "$" + item.getLineTotal().getAmount().toPlainString(),
                            TextAlignment.RIGHT));
        }
    ...
    private void addPaymentHistory(
            Document document, java.util.List<com.osgiliath.domain.payment.Payment> payments) {
        // Add section header
        Paragraph paymentHeader =
                new Paragraph("Payment History")
                        .setFontSize(12)
                        .setBold()
                        .setMarginBottom(10)
                        .setMarginTop(10);
        ...
        for (com.osgiliath.domain.payment.Payment payment : payments) {
            table.addCell(
                    createCell(
                            payment.getPaymentDate().format(DATE_FORMATTER), TextAlignment.LEFT));
            table.addCell(
                    createCell(
                            payment.getPaymentMethod().toString().replace("_", " "),
                            TextAlignment.LEFT));
            table.addCell(
                    createCell(
    ...
    private void addTotals(Document document, Invoice invoice) {
        ...
        totalsTable.addCell(
                createTotalValueCell("$" + invoice.getSubtotal().getAmount().toPlainString()));

        // Tax
        totalsTable.addCell(createTotalLabelCell("Tax:"));
        totalsTable.addCell(
                createTotalValueCell("$" + invoice.getTaxAmount().getAmount().toPlainString()));

        // Total
        totalsTable.addCell(createTotalLabelCell("Total:"));
        totalsTable.addCell(
                createTotalValueCell("$" + invoice.getTotalAmount().getAmount().toPlainString()));
        ...
        Cell balanceValue =
                createTotalValueCell("$" + invoice.getBalanceDue().getAmount().toPlainString());
    ...
    private Cell createCell(String text, TextAlignment alignment) {
        return new Cell().add(new Paragraph(text)).setTextAlignment(alignment).setFontSize(10);
    }
