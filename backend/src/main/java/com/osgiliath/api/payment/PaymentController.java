...
/**
 * REST Controller for Payment operations Manages payment recording and retrieval with automatic
...
public class PaymentController {
    ...
    @Operation(
            ...
            description =
                    "Records a payment against an invoice. Validates invoice status (must be SENT), "
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        responseCode = "422",
                        description =
                                "Business rule violation - invoice not in SENT status or payment exceeds balance")
            })
    public ResponseEntity<PaymentResponse> recordPayment(
            @Parameter(description = "Invoice ID", required = true) @PathVariable UUID invoiceId,
        ...
        Payment payment =
                getPaymentByIdQueryHandler.handle(new GetPaymentByIdQuery(result.getPaymentId()));
