...
/** Email Service using AWS SES Handles sending invoice emails with PDF attachments */
...
public class EmailService {
    ...
    /**
     * Send invoice email asynchronously This method runs in a separate thread to not block the
     * transaction
     */
    @Async
    public void sendInvoiceEmail(Invoice invoice, Customer customer, byte[] pdfAttachment) {
        try {
            log.info(
                    "Sending invoice email for invoice: {} to customer: {}",
            ...
            log.error(
                    "Failed to send invoice email for invoice: {}. Error: {}",
                    invoice.getInvoiceNumber(),
                    e.getMessage(),
                    e);
        }
        ...
        try {
            ...
            String rawMessage =
                    buildRawEmailMessage(
                            toEmail, subject, htmlBody, textBody, attachment, attachmentFileName);

            RawMessage rawEmailMessage =
                    RawMessage.builder().data(SdkBytes.fromUtf8String(rawMessage)).build();

            SendRawEmailRequest rawEmailRequest =
                    SendRawEmailRequest.builder().rawMessage(rawEmailMessage).build();
    ...
    private String buildRawEmailMessage(
        ...
        message.append("Content-Type: multipart/alternative; boundary=\"")
                .append(altBoundary)
                .append("\"\n\n");
        ...
        // PDF attachment
        message.append("--").append(boundary).append("\n");
        message.append("Content-Type: application/pdf; name=\"")
    ...
    private String buildEmailHtmlBody(Invoice invoice, Customer customer) {
        ...
        html.append(
                ".total-box { background-color: #e8f4f8; padding: 15px; border-radius: 5px; margin: 20px 0; font-weight: bold; }");
        html.append(
                ".footer { margin-top: 30px; padding-top: 20px; border-top: 2px solid #ddd; font-size: 12px; color: #777; text-align: center; }");
        html.append("</style>");
        html.append("</head>");
        html.append("<body>");
        ...
        html.append("<p><strong>Invoice Number:</strong> ")
                .append(invoice.getInvoiceNumber())
                .append("</p>");
        html.append("<p><strong>Issue Date:</strong> ")
                .append(invoice.getIssueDate().format(DATE_FORMATTER))
                .append("</p>");
        html.append("<p><strong>Due Date:</strong> ")
        ...
        for (LineItem item : invoice.getLineItems()) {
            ...
            html.append("<td>")
                    ...
                    .append("</td>");
            html.append("<td>$")
                    .append(item.getUnitPrice().getAmount().toPlainString())
                    .append("</td>");
            html.append("<td>$")
                    .append(item.getLineTotal().getAmount().toPlainString())
                    .append("</td>");
            html.append("</tr>");
        }
        ...
        html.append("<p><strong>Subtotal:</strong> $")
                .append(invoice.getSubtotal().getAmount().toPlainString())
                .append("</p>");
        html.append("<p><strong>Tax:</strong> $")
                .append(invoice.getTaxAmount().getAmount().toPlainString())
                .append("</p>");
        html.append(
                        "<p style=\"font-size: 18px; color: #2c3e50;\"><strong>Total Amount:</strong> $")
                .append(invoice.getTotalAmount().getAmount().toPlainString())
                .append("</p>");
        html.append("<p style=\"font-size: 18px; color: #e74c3c;\"><strong>Balance Due:</strong> $")
                .append(invoice.getBalanceDue().getAmount().toPlainString())
                .append("</p>");
        html.append("</div>");
    ...
    private String buildEmailTextBody(Invoice invoice, Customer customer) {
        ...
        text.append("Tax: $")
                .append(invoice.getTaxAmount().getAmount().toPlainString())
                .append("\n");
        text.append("Total Amount: $")
                .append(invoice.getTotalAmount().getAmount().toPlainString())
                .append("\n");
        text.append("Balance Due: $")
                .append(invoice.getBalanceDue().getAmount().toPlainString())
                .append("\n\n");
